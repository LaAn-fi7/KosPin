    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('owner.store-kos') }}" method="POST" enctype="multipart/form-data" id="form-kos">
                        @csrf
                            ...
                            <div class="col-12 mb-3">
                                <select name="district" class="form-select">
                                    ...
                                    @foreach(config('palembang.districts') as $d)
                                        <option value="{{ $d }}" {{ old('district',$kos->district ?? '') == $d ? 'selected' : '' }}>{{ $d }}</option>
                                    @endforeach
                                </select>
                                @error('district')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <h6 class="fw-bold text-primary mb-3">
                                    <i class="fas fa-images me-2"></i>Gambar Kos
                                </h6>

                                <label for="images" class="form-label fw-semibold">Unggah Gambar (maks 10 gambar, masing-masing ≤ 5MB)</label>
                                <input type="file" class="form-control @error('images.*') is-invalid @enderror" 
                                       id="images" name="images[]" multiple accept="image/*">
                                @error('images.*')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                                <small class="text-muted d-block mb-2">Pilih beberapa gambar untuk ditampilkan di halaman kos.</small>

                                <!-- Thumbnails preview sebelum submit -->
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start mb-3">
                                            <div class="d-flex align-items-start gap-3">
                                                <div id="previewCoverWrapper" style="width:120px; height:90px; overflow:hidden; border-radius:6px; background:#f5f5f5;">
                                                    <img id="previewCover" src="" alt="cover" style="width:100%; height:100%; object-fit:cover; display:none;">
                                                    <div id="previewCoverPlaceholder" class="d-flex justify-content-center align-items-center text-muted" style="width:100%; height:100%;">No Image</div>
                                                </div>
                                                <div>
...
function showPreview() {
    ...
    const district = $('#district').val() || 'Kecamatan';
    ...
    // Jika ada gambar dipilih, gunakan gambar pertama sebagai cover preview
    ...
    if (files && files.length > 0) {
        ...
        $('#previewCoverPlaceholder').hide();
    } else {
        $('#previewCover').hide().attr('src','');
        $('#previewCoverPlaceholder').show();
    }
...
$('#name, #district, #city, #province, #price_per_month, #description, #gender, #total_rooms').on('input change', function() {
    if ($('#previewSection').is(':visible')) {
        showPreview();
    }
});

// Format price input (hanya angka)
$('#price_per_month').on('input', function() {
    const value = $(this).val().replace(/[^0-9]/g, '');
    $(this).val(value);
});

// Thumbnails preview untuk input images[]
...
// Karena input file tidak mudah diubah satu-per-satu, kita simpan FileList ke array sementara
let tempFiles = [];

function updateThumbnails() {
    clearThumbnails();
    tempFiles.forEach((file, idx) => {
        thumbnails.appendChild(createThumb(file, idx));
    });
}

function removeFileAtIndex(index) {
    if (index < 0 || index >= tempFiles.length) return;
...
function syncInputFiles() {
    // Convert array to DataTransfer untuk assign ke input.files
...
imagesInput.addEventListener('change', function(e) {
    ...
    // Gabungkan existing tempFiles + new files, tapi batasi MAX_FILES
    tempFiles = tempFiles.concat(files).slice(0, MAX_FILES);

    // Validasi ukuran tiap file
    for (let i = 0; i < tempFiles.length; i++) {
        if (tempFiles[i].size > MAX_SIZE) {
            alert('Salah satu file melebihi 5MB: ' + tempFiles[i].name);
    ...
    if (tempFiles.length > MAX_FILES) {
        ...
        tempFiles = tempFiles.slice(0, MAX_FILES);
    }

    syncInputFiles();
    updateThumbnails();

    // Jika preview dibuka, refresh cover
    if ($('#previewSection').is(':visible')) {
        showPreview();
    }
});

// Sebelum submit, opsi: cek lagi batasan
document.getElementById('form-kos').addEventListener('submit', function(e) {
    if (tempFiles.length > MAX_FILES) {
        e.preventDefault();
        alert('Maksimal ' + MAX_FILES + ' gambar.');
        return false;
    ...
    for (let f of tempFiles) {
        if (f.size > MAX_SIZE) {
            e.preventDefault();
            alert('Salah satu file melebihi 5MB: ' + f.name);
            return false;
        }
    }
    // Submit akan melanjutkan dengan input.files yang sudah disinkronkan
});
</script>
@endsection
